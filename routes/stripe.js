...
const CancelStripeSubscription = async ({ userId, feedbackMessage, feedbackPoint }) => {

  await User.findOneAndUpdate({ _id: userId }, { $set: { feedbackMessage, feedbackPoint }});
...
const verifyUserStatus = (userId) => new Promise((res) => {
  let retry = 0;
...
router.post('/create-user-subscription', authenticateAuthToken, async (req, res) => {
  try {
    const { user: { _id: userId }, body: { token, coupon, planId, userData, selectedPlan } } = req;
    ...
    const user = await User.findOne({ _id: userId });
    const { status } = user;
    if (subscriptionData?.id && status === 'Subscribed') { res.status(200).json({ status: true, user, subscriptionData }); }
...
router.post('/cancel-subscription', authenticateAuthToken, async (req, res) => {
  try {
    const { user: { _id: userId }, body: { feedbackMessage, feedbackPoint } } = req;
...
router.post('/update-customer-card', authenticateAuthToken, async (req, res) => {
  try {
    const { user: { _id: userId }, body: { token, coupon } } = req;
